...
int addNew(tree* t, char* val) {
    ...
    if (strcmp(val, (*t) -> val) < 0) return addNew(&(*t) -> left, val);
    if (strcmp(val, (*t) -> val) > 0) return addNew(&(*t) -> right, val);
    return 0;
}
...
void printTreePretty(tree t, int level) {
    ...
    // Специальная обработка унарного минуса
    if (strcmp(getValue(t), "~") == 0) {
        printf("-\n");
    } else {
        printf("%s\n", getValue(t));
    }
...
void deleteUnitMultiply(tree* t) {
    ...
    // Сначала обрабатываем потомков
    deleteUnitMultiply(&(*t)->left);
    deleteUnitMultiply(&(*t)->right);

    // Только для оператора умножения
    if (strcmp((*t)->val, "*") == 0) {
        tree left = (*t)->left;
        tree right = (*t)->right;
        
        // Левый операнд равен 1
        if (left != NULL && strcmp(left->val, "1") == 0) {
            tree to_remove = *t;
            *t = right; // Заменяем текущий узел на правого потомка
            destroyTree(left); // Удаляем левую 1
            free(to_remove->val);
            free(to_remove); // Удаляем старый узел *
        } else if (right != NULL && strcmp(right->val, "1") == 0) {
            tree to_remove = *t;
            *t = left; // Заменяем текущий узел на левого потомка
            destroyTree(right); // Удаляем правую 1
            free(to_remove->val);
            free(to_remove); // Удаляем старый узел *
        }
    }
